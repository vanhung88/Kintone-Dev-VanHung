...
(function() {
    ...
    var changeDrafter = function (department) {
      ...
      switch (department) {
          case "管理部":
            departDrafter = [{code: 'bap-kintone-developer@bap.jp', name: '渡辺 健一'}];
            break;
          case "営業部":
            departDrafter = [{code: 'bap-kintone-developer@bap.jp', name: '田中 信'}];
            break;
          case "情報システム部":
            departDrafter = [{code: 'bap-kintone-developer@bap.jp', name: '望月 泉'}];
            break;
          case "セキュリティ委員会":
            departDrafter = [{code: 'bap-kintone-developer@bap.jp', name: '田中 信'}];
            break;
    ...
    var changefirstApproval = function (department) {
      ...
      switch (department) {
          case "管理部":
            firstApproval = [{code: 'bap-kintone-developer@bap.jp', name: '田中 信'}];
            break;
          case "営業部":
            firstApproval = [{code: 'bap-kintone-developer@bap.jp', name: '渡辺 健一'}];
            break;
          case "情報システム部":
            firstApproval = [{code: 'bap-kintone-developer@bap.jp', name: '渡辺 健一'}];
            break;
    ...
    var documentStamped = [
      ...
      if (record.捺印詳細.value[0].value.印章.value !== 'なし') {
        if (
            ...
            (user.code === record.決裁承認実行者.value[0].code && record.ステータス.value === '再捺印') ||
